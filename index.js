...
app.get('/api/menu', async (req, res) => {
    try {
        // Return ALL items (available + unavailable). The frontend already
        // handles showing unavailable items greyed-out with a "Not Available"
        // tag (Swiggy-style) instead of hiding them — filtering here was
        // hiding them outright, which made an "edit -> mark unavailable"
        // look like the item had been deleted.
        const [rows] = await getDB().execute(
...
app.post('/api/orders', auth, async (req, res) => {
    ...
    try {
        ...
        // Guard against ordering items that are now unavailable — the menu
        // endpoint returns unavailable items too (so students can see them
        // greyed-out), so this check stops anyone bypassing the UI and
        // posting an order for one directly.
        const itemIds = items.map(i => i.id);
        const [availableRows] = await pool.query(
            'SELECT id, name FROM menu_items WHERE id IN (?) AND is_available = 1',
            [itemIds]
        );
        const availableIds = new Set(availableRows.map(r => r.id));
        const unavailable   = items.filter(i => !availableIds.has(i.id));
        if (unavailable.length) {
            return res.status(400).json({
                error: `These items are no longer available: ${unavailable.map(i => i.name || i.id).join(', ')}. Please remove them from your cart.`
            });
        }
